...
describe("MINT AND CLAIM\n\n", function () {
  ...
  it("Mint token", async function () {
    ...
    await valocracy.mintValocracy(OTHER_HOLDER.address,1,"dale nftezinho 2",10);
    await valocracy.mintValocracy(THIRD_HOLDER.address,1,"dale nftezinho 3",10);
  ...
  it("owner balance usdt", async function () {
    const ownerBalanceUSDT = await USDTContract.balanceOf(OWNER.address);
    console.log('QUANTIDADE USDT DO OWNER -->',ethers.formatUnits(ownerBalanceUSDT,18)); 
  ...
  it("claim", async function () {
    ...
    await valocracy.claim(2n,USDTAddress);
  ...
  it("Get my tokens ids", async function () {
    const tokens = await valocracy.getOwnerTokensEconomics(OWNER.address);
    console.log('NFTS ECONOMICS IDs do OWNER PÓS CLAIM -->',tokens);
  ...
  it("after burn", async function () {
    const token2 = await valocracy.tokenURI(2n);
    console.log('BUSCA INFORMAÇÃO DO TOKEN QUEIMADO ->>',token2);
