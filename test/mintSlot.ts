...
describe("ECONOMIC TOKEN DATA\n\n", function () {
  ...
  it("All tokens", async function () {
    ...
    const tokens = await valocracy.fetchAllTokens(0,1,true);
  ...
  it("Mint token OWNER", async function () {
    ...
    console.log("MINTANDO SLOT NFT OWNER")

    const mintArray: MintData[] = [
      {
        to:OWNER.address,
        tokenURI:"dale nftezinho 1",
        rarity:10
      },
      {
        to:OWNER.address,
        tokenURI:"dale nftezinho 2",
        rarity:10
      }
    ]

    await valocracy.mintSlot(mintArray);
  ...
  */
  ...
  it("Get OWNER tokens ids", async function () {
    expect( await valocracy.fetchOwnerTokens(OWNER.address,0,3,true)).to.deep.equal([ [ 1n, 2n ], [ 3n, 4n ] ]);

    const tokens = await valocracy.fetchOwnerTokens(OWNER.address,0,0,true);
  ...
  it("Get OWNER tokens ids", async function () {
    ...
    const tokens = await valocracy.fetchOwnerTokens(OWNER.address,0,2,false);
    console.log('NFTS IDs do OWNER DESCENDENTE-->',tokens);
    expect( await valocracy.fetchOwnerTokens(OWNER.address,0,2,false)).to.deep.equal([ [ 3n, 4n ] , [ 1n, 2n ] ]);
  ...
  it("All tokens", async function () {
    ...
    const tokens = await valocracy.fetchAllTokens(0,10,true);
    console.log('TODOS OS TOKENS -->',tokens);
    expect( await valocracy.fetchAllTokens(0,10,true)).to.deep.equal([ [ 1n, 2n ], [ 3n, 4n ] , [ 5n, 6n ] , [ 7n, 8n ] ]);
  ...
  it("All token", async function () {
    ...
    const tokens = await valocracy.fetchAllTokens(0,10,false);
    console.log('TODOS OS TOKENS REVERSO-->',tokens);
    expect( await valocracy.fetchAllTokens(0,10,false)).to.deep.equal([[ 7n, 8n ] , [ 5n, 6n ], [ 3n, 4n ] , [ 1n, 2n ] ]);
